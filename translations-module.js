...
const TranslationsModule = (() => {
  ...
  async function init() {
    ...
    initPromise = (async () => {
      try {
        translationsMetadata = await fetchJSON("translations-index.json?v=20260520-net");
        loadFromCache();
      } catch (error) {
  ...
  // Expanded Greek lexicon (Abbott-Smith via STEPBible, CC BY 4.0), keyed by
  // Strong's number. Lazy-loaded once and cached. Returns the entry for one
  // number or null. Generated by scripts/build-greek-lexicon.js.
  let greekLexiconCache = null;
  let greekLexiconPromise = null;

  async function loadGreekLexicon() {
    if (greekLexiconCache) return greekLexiconCache;
    if (greekLexiconPromise) return greekLexiconPromise;
    greekLexiconPromise = (async () => {
      try {
        greekLexiconCache = await fetchJSON("lexicons/strongs-lexicon.json");
      } catch (e) {
        console.warn("Greek lexicon not available:", e);
        greekLexiconCache = {};
      }
      return greekLexiconCache;
    })();
    return greekLexiconPromise;
  }

  async function getGreekLexicon(strongsNum) {
    const lex = await loadGreekLexicon();
    return lex[strongsNum] || null;
  }
  ...
  // ---------------------------------------------------------------------------
  // NET Bible footnote loading (per-book lazy loading)
  // ---------------------------------------------------------------------------

  const netNotesCache = {};

  async function loadNetNotes(bookKey) {
    if (netNotesCache[bookKey]) return netNotesCache[bookKey];
    const resp = await fetch(`translations/net-notes/${encodeURIComponent(bookKey)}.json`);
    if (!resp.ok) throw new Error(`${resp.status}: net-notes/${bookKey}.json`);
    const notes = await resp.json();
    netNotesCache[bookKey] = notes;
    return notes;
  }

  async function getNetNote(noteId, bookKey) {
    const notes = await loadNetNotes(bookKey);
